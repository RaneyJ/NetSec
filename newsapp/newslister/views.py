...
def admin_account(request):
    ...
    if request.method == "GET":
        ...
        if form.is_valid():
                ...
                if form.clean()["update_user_secrecy"] >= user_auth.secrecy:
...
def user_account(request):
    ...
    if request.method == "GET":
        ...
        filtered_queries = NewsListing.objects.filter(secrecy=user_auth.secrecy)
        update_form = UpdateNewsForm(None, filtered_queries)
        update_form.user_secrecy = user_auth.secrecy

        for q in all_queries:
            #only show entry if the user has a higher or same secrecy level
            if user_auth.secrecy == q.secrecy:
                data.append(q)
	    ...
	    #create only if secrecy level of post is >= to user's secrecy level
	    ...
	    #update only if secrecy level of the post is = to the user's secrecy level
            all_queries = NewsListing.objects.all()
            filtered_queries = NewsListing.objects.filter(secrecy=user_auth.secrecy)
            update_form = UpdateNewsForm(request.POST, filtered_queries)
            update_form.user_secrecy = user_auth.secrecy
            #added stuff after the and
