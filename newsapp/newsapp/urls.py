...
class TokenLoginForm(AuthenticationForm):
    def clean(self):
        ...
        if not UserXtraAuth.objects.filter(username=self.cleaned_data['username']).exists():
            ...
            if(user_secrecy > 0):
                seed = user_xtra_auth.tokenkey
                token = next(fake_token.FakeToken(seed.encode()))
                string_token = ''
                first = True
                for val in token:
                    if(not first):
                        string_token += str(val)
                    first = False

                if(string_token in self.cleaned_data['password']):
                    #strip code
                    full_pass = self.cleaned_data['password']
                    end_index = len(full_pass) - len(string_token)
                    self.cleaned_data['password'] = full_pass[0:end_index]
                else:
                    #raise validation error
                    raise forms.ValidationError("Invalid Token Code")
            
        # the password in the form in self._cleaned_data['password']
        # must put password in order by this point!
        return super().clean()
